...
<del>一部分代码（主要是简洁模式相关）因失误忘记上传了，留在了学校，这段时间的更新会因此推迟一阵时间。

而且这部分代码重新写挺麻烦的，效果也可能出现偏差。

所以打算先**专注其他功能的开发以及毕设**，等回到学校后合并代码，再考虑发布新版本~</del>

因为短时间返回不了学校，靠着残留的部分代码重新写了一遍简洁模式。

并且下次更新还会有**WebDAV备份、自定义属性图标和名称**等功能。

也修复了用户反馈的桌面小部件的颜色问题、选中模式下点击小圆圈的优化等。

**本周内应该就会更新了~敬请期待。**

 {% asset_img delay.jpg %}

 {% asset_img delay1.png %}

 {% asset_img delay2.png %}

 {% asset_img delay3.jpg %}
