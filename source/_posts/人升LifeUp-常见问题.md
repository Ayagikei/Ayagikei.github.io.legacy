...
1/8-1/9号检测到了数位用户遇到了QQ登录崩溃的问题，**经查询发现是QQ互联那边的接口返回值不正常**。（有一个过期时间的数值应该是int，结果这几位用户遇到了返回负long值的问题）

**1.69.2版本已经修复该问题，并且升级了相关SDK版本。**
...
目前，我们官方上架并且维护更新的应用市场有：**酷安、魅族、小米、Google Play、应用宝、阿里应用分发（含：豌豆荚、PP 助手、YunOS 应用商店、UC应用商店等）**。
